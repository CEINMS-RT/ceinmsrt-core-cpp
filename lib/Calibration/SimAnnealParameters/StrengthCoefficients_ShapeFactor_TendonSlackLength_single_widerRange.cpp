...
StrengthCoefficients_ShapeFactor_TendonSlackLength_single_widerRange<NMSmodelT>
::StrengthCoefficients_ShapeFactor_TendonSlackLength_single_widerRange ( NMSmodelT& subject,
...
//  cout << endl;
	...
	noParameters_ = musclesIndexToCalibrate_.size() + strengthCoefficientValues_.size() + musclesIndexToCalibrate_.size() + musclesIndexToCalibrate_.size(); // LsT, Lom, strength Coeff and shape factor
	SyncToolsCal::Shared::strengthCoeff = strengthCoefficientValues_.size();
	SyncToolsCal::Shared::strengthCoeffReady.notify();
}


template <typename NMSmodelT>
void StrengthCoefficients_ShapeFactor_TendonSlackLength_single_widerRange<NMSmodelT>
...
::setUpperLowerBounds ( vector<double>& upperBounds, vector<double>& lowerBounds )
{
	...
	for ( int i = 0; i < musclesIndexToCalibrate_.size(); ++i )
	{
		vector<double> optimalFiberLengths;
		subject_.getOptimalFiberLengths ( optimalFiberLengths );
		upperBounds.at ( indexCounter + musclesIndexToCalibrate_.size() + i ) =  optimalFiberLengths.at ( musclesIndexToCalibrate_.at ( i ) ) + ( optimalFiberLengths.at ( musclesIndexToCalibrate_.at ( i ) ) * 0.25 );
		lowerBounds.at ( indexCounter + musclesIndexToCalibrate_.size() + i ) =  optimalFiberLengths.at ( musclesIndexToCalibrate_.at ( i ) ) - ( optimalFiberLengths.at ( musclesIndexToCalibrate_.at ( i ) ) * 0.25 );
	}
...
template <typename NMSmodelT>
std::ostream& operator<< ( std::ostream& output, const StrengthCoefficients_ShapeFactor_TendonSlackLength_single_widerRange<NMSmodelT>& p )
{
