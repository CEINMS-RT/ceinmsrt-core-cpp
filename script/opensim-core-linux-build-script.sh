...
if [[ $OS_NAME == *"Debian"* ]]; then
   ...
   # Export JAVA_HOME variable.
   export JAVA_HOME=$(dirname $(dirname $(dirname $(realpath /usr/bin/java))))
fi
echo
...
cmake ~/opensim-workspace/opensim-core-source/dependencies -DCMAKE_INSTALL_PREFIX=~/opensim-workspace/opensim-core-dependencies-install/ -DSUPERBUILD_ezc3d=on -DOPENSIM_WITH_CASADI=$MOCO -DCMAKE_BUILD_TYPE=$DEBUG_TYPE
cmake . -LAH
cmake --build .
echo
